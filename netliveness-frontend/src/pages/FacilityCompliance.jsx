...
import { 
  ShieldAlert, Search, CircleHelp, RefreshCcw, Download, Upload, FilePlus
} from 'lucide-react';
import { getFacilityRequirements, updateFacilityRequirement, seedFacilityRequirements, uploadFacilityDocument, downloadFacilityDocument } from '../api';
...
export default function FacilityCompliance() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const data = await getFacilityRequirements();
  ...
  const handleSeed = async () => {
    try {
      ...
      await seedFacilityRequirements();
      toast.success("Tesis Güvenliği listesi başarıyla oluşturuldu.", { id: 'seed' });
      loadData();
    } catch {
  ...
  const handleUpdate = async (req) => {
    try {
      setSavingId(req.id);
      await updateFacilityRequirement(req.id, req);
  ...
  const handleFileUpload = async (id, file) => {
    try {
      ...
      await uploadFacilityDocument(id, file);
  ...
  const handleDownload = (id, fileName) => {
    if (!fileName) return;
    const url = downloadFacilityDocument(id);
  ...
  return (
    <div className="page-container" style={{ padding: '24px 24px 40px', maxWidth: '100%', overflowX: 'hidden' }}>
      <div className="page-header" style={{ marginBottom: '32px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '20px' }}>
          <div style={{ flex: '1', minWidth: '300px' }}>
            <h1 style={{ fontSize: '24px', fontWeight: 600, color: 'var(--text-primary)', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '12px' }}>
              <ShieldAlert color="var(--accent-green)" size={28} />
              Tesis Güvenliği Uyum Takibi
            </h1>
            <p style={{ color: 'var(--text-secondary)', fontSize: '14px', marginBottom: '20px' }}>
              REPKON Yerleşke ve Tesis Güvenliği Standartları Kontrol Listesi
            </p>
          ...
          <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
            <a 
              href="/templates/Facility_Security_Template.md" 
              download="Facility_Security_Template.md"
            ...
            <div className="search-box" style={{ 
              ...
              <input 
                type="text" 
                placeholder="Kontrol veya tema ara..." 
                value={searchTerm}
        ...
        {requirements.length === 0 && (
          <div style={{ 
            ...
            <h3 style={{ marginBottom: '8px' }}>Tesis Güvenliği Kontrol Listesi Boş</h3>
            <p style={{ color: 'var(--text-secondary)', marginBottom: '24px' }}>Tesis güvenlik kontrollerini takip etmeye başlamak için listeyi oluşturun.</p>
      ...
      <ComplianceDocManager standard="FACILITY" />
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
                    <thead>
                      <tr style={{ textAlign: 'left', background: 'rgba(255,255,255,0.02)', borderBottom: '1px solid var(--border-color)' }}>
                        <th style={{ padding: '16px', fontSize: '13px', color: 'var(--text-secondary)', width: '80px' }}>No</th>
                        <th style={{ padding: '16px', fontSize: '13px', color: 'var(--text-secondary)', width: 'auto' }}>Kontrol Açıklaması</th>
