...
import { getIsoRequirements, updateIsoRequirement, seedIsoRequirements, uploadIsoDocument, downloadIsoDocument } from '../api';
...
export default function IsoCompliance() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const data = await getIsoRequirements();
  ...
  const handleSeed = async () => {
    try {
      ...
      await seedIsoRequirements();
      toast.success("ISO listesi başarıyla oluşturuldu.", { id: 'seed' });
      loadData();
    } catch {
  ...
  const handleUpdate = async (req) => {
    try {
      setSavingId(req.id);
      await updateIsoRequirement(req.id, req);
  ...
  const handleFileUpload = async (id, file) => {
    try {
      ...
      await uploadIsoDocument(id, file);
  ...
  const handleDownload = (id, fileName) => {
    if (!fileName) return;
    const url = downloadIsoDocument(id);
  ...
  return (
    <div className="page-container" style={{ padding: '24px 24px 40px', maxWidth: '100%', overflowX: 'hidden' }}>
      <div className="page-header" style={{ marginBottom: '32px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '20px' }}>
          <div style={{ flex: '1', minWidth: '300px' }}>
            <h1 style={{ fontSize: '24px', fontWeight: 600, color: 'var(--text-primary)', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              ISO 27001:2022 Uyum Takibi
            </h1>
            <p style={{ color: 'var(--text-secondary)', fontSize: '14px', marginBottom: '20px' }}>
              REPKON Bilişim Sistemleri Bilgi Güvenliği Yönetim Sistemi Kontrolleri
            </p>
          ...
          <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
            <a 
              href="/templates/ISO27001_Compliance_Template.md" 
              download="ISO27001_Compliance_Template.md"
        ...
        {requirements.length === 0 && (
          <div style={{ 
            ...
            <h3 style={{ marginBottom: '8px' }}>ISO 27001 Kontrol Listesi Boş</h3>
            <p style={{ color: 'var(--text-secondary)', marginBottom: '24px' }}>Bilgi güvenliği kontrollerini takip etmeye başlamak için listeyi oluşturun.</p>
      ...
      <ComplianceDocManager standard="ISO27001" />
