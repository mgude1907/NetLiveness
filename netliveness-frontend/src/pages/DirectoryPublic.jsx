...
export default function DirectoryPublic() {
  ...
  useEffect(() => {
    ...
    style.innerHTML = `
      html, body { background: #000 !important; margin: 0; padding: 0; min-height: 100vh; overflow-x: hidden; }
      #root { 
        background: #000 url('/bg-login-3.jpg') no-repeat fixed center center / cover !important; 
        min-height: 100vh;
        display: flex;
        flex-direction: column;
      }
      .page-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.85); z-index: 1; pointer-events: none; }
  ...
  const load = useCallback(async () => {
    try {
      const [dir, surveys] = await Promise.all([getDirectoryEntries(), getActiveSurveys()]);
      setEntries(dir);
  ...
  if (loading) return (
    <div style={{ width: '100vw', height: '100vh', background: '#000', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 15 }}>
      <div className="spinner" style={{ width: 36, height: 36, borderTopColor: '#f59e0b', borderWidth: 3 }} />
      <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.4)', letterSpacing: 3, fontWeight: 900 }}>REPKON</span>
    </div>
  );
    ...
    <div style={{ width: '100%', minHeight: '100vh', fontFamily: "'Inter', sans-serif", color: '#fff', display: 'flex', flexDirection: 'column', position: 'relative', zIndex: 2 }}>
      ...
      <div style={{ position: 'relative', width: '100%', minHeight: '300px', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 5, padding: '30px 0' }}>
        <div style={{ maxWidth: 800, width: '100%', padding: '0 40px', textAlign: 'center', position: 'relative', zIndex: 10 }}>
          ...
          <h1 style={{ fontSize: 'max(3.2vw, 28px)', fontWeight: 900, marginBottom: 12, letterSpacing: -1.5, color: '#fff' }}>
            Kurumsal <span style={{ color: '#f59e0b' }}>Rehber</span>
          </h1>
          <div style={{ background: 'rgba(255,255,255,0.03)', backdropFilter: 'blur(30px)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 20, padding: '8px 8px 8px 24px', display: 'flex', alignItems: 'center', gap: 14, boxShadow: '0 20px 40px rgba(0,0,0,0.5)', maxWidth: 540, margin: '0 auto' }}>
            <Search size={18} color="#f59e0b" strokeWidth={3} />
            <input style={{ background: 'none', border: 'none', color: '#fff', fontSize: 16, flex: 1, outline: 'none', fontWeight: 600 }} placeholder="Hızlı ara..." value={search} onChange={e => setSearch(e.target.value)} />
...
function StatBox({ icon: Icon, label, value, color }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      ...
      <div>
        <div style={{ fontSize: 8, fontWeight: 800, color: 'rgba(255,255,255,0.2)', letterSpacing: 1 }}>{label}</div>
        <div style={{ fontSize: 16, fontWeight: 900, lineHeight: 1 }}>{value}</div>
      </div>
    </div>
  );
}
    ...
    >
      ...
      <div style={{
        ...
        {entry.imageUrl
          ...
          : <User size={28} color="rgba(255,255,255,0.1)" />
        }
      </div>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 5, color: 'rgba(255,255,255,0.3)', fontSize: 11, fontWeight: 500 }}>
...
function MiniContact({ icon: Icon, text, color }) {
  if (!text || text === '-' || text === 'null') {
    return <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', gap: 8 }}>
